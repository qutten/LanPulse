...
if exist "%~dp0dist\lanpulse-server.exe" (
    "%~dp0dist\lanpulse-server.exe" %*
    pause
    exit /b
)

if exist "%~dp0server\lanpulse-server.exe" (
    "%~dp0server\lanpulse-server.exe" %*
    pause
    exit /b
)
...
if %errorlevel%==0 (
    ...
    echo 请任选一种方式继续：
    echo   1. 使用独立版：把 dist\lanpulse-server.exe 复制到本机，
